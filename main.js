var movies = [
	...
	},
	{
		...
		bigPoster: "http://i3.ytimg.com/vi/kjC1zmZo30U/maxresdefault.jpg",
		smallPoster: "http://i3.ytimg.com/vi/kjC1zmZo30U/hqdefault.jpg",
	},
];
const elList = document.querySelector("#list");
var inputPoster = movies[0].bigPoster;
var inputTitle = movies[0].title;
var inputText = movies[0].summary;
var inputLink = movies[0].trailer;
var inputRating = movies[0].imdbRating;

for (let i = 1; i < 11; i++) {
	let newLi = `
		<div class="card lh-1 mt-5" style="width: 14rem">
		<span></span>
			<img src="${inputPoster}" class="card-img-top" style="height: 150px;" />
			<div class="card-body">
				<h5 class="card-title mt-3 text-white">${inputTitle}</h5>
				<p class="card-text text-white">${inputText}</p>
				<a href="${inputLink}" class="button">Trailer ko'rish</a>
				<p class="p text-white">Rating: ${inputRating}</p>
			</div>
		</div>
`;
	inputPoster = movies[i].smallPoster;
	inputTitle = movies[i].title;
	inputText = movies[i].summary;
	inputLink = movies[i].trailer;
	inputRating = movies[i].imdbRating;
